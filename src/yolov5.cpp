...
YOLOv5Detector::YOLOv5Detector(const std::string& model_path, 
    ...
    std::cout << "YOLOv5 模型加载成功" << std::endl;
}
...
std::vector<Detection> YOLOv5Detector::detect(const cv::Mat& image) {
    ...
    try {
        ...
        // 后处理
        return postprocess(output_tensors[0], image, input_width, input_height);
...
std::vector<Detection> YOLOv5Detector::postprocess(const Ort::Value& output_tensor,
